...
class DataWebSocket(WebSocket):
    clients = set()
    h_clients = {}
    free_room_ids = ["APIDEMO","MSH_USER","OFFICE","HOLLOW_USER","LLM","INROOM","ENTERROOM"]
    @staticmethod
        ...
        if len(info) == 4:
            if info[0] == "JOINMOREROOMS":
                for aim_id in info[3]:
                    will_return = True
                    if aim_id not in DataWebSocket.free_room_ids:
                        for startswith_str in ["MSH_USER","HOLLOW_USER","LLM"]:
                            if aim_id.startswith(startswith_str):
                                will_return = False
                    if will_return:
                        return
                    h_clients_base = set()
    ...
    def send_to_target_room(message,target_room):
        info = json_decode(message)
        if len(info) == 3:
            print("send_to_target_room message ok")
            print("send_to_target_room target_room:",len(DataWebSocket.h_clients.get(target_room,[])))
